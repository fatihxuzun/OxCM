...
Vtemp = FunctionSpace(mesh, "CG", int(dg)) # dg determines the number of dof points
...
fInt = sc.RectBivariateSpline(y, x, zz, kx=3, ky=3)
...
uDof = []
for i in fInd:
    uDof.append(fInt.ev(fCoord[i][1] + fix_y, fCoord[i][0] + fix_x))
...
for i in range(len(fInd)):
    ...
    bcLine = 'bc = bc + [DirichletBC(V.sub(2), Constant((' + str(-uDof[i]) + ')), cnt_boundary' + str(i) + ', method="pointwise"' + ')]'
    bc.append(exec(bcLine.splitlines()[0]))

def base_boundary(co):
    return (-(P1_z * 1e-3) + P1_z) < round(co[2],3) < ((P1_z * 1e-3) + P1_z)
